...
class InstagramBot {
    ...
    async initPuppeter() {
        ...
        this.page.setViewport({width: 1300, height: 764});
    }
    ...
    async _doPostLikeAndFollow (parentClass, page){
        ...
        for (let r = 1; r < 4; r++) {//loops through each row
            for (let c = 1; c < 4; c++) {//loops through each item in the row
                ...
                await page.waitFor(1000 + Math.floor(Math.random() * 250));//wait for random amount of time
                ...
                console.log(`INTERAGINDO COM O POST DE ${username} `);
                ...
                if (hasEmptyHeart !== null && Math.random() < this.config.settings.like_ratio) {
                    await page.click(this.config.selectors.post_like_button)//click the like button
                        .catch((e) => {console.log('<<< ERRO SELETOR NAO ENCONTRADO >>> ' + e.message)
                                        return null});
                ...
                if (followStatus === 'Follow' && !isArchivedUser) {
                    await this.firebase_db.addFollowing(username).then(() => {
                        ...
                        console.log('<<< COMECOU A SEGUIR  >>> ' + username);
                        return page.waitFor(10000 + Math.floor(Math.random() * 5000));
                    }).catch((e) => {
                        console.log('<<< JA ESTA SEGUINDO  >>> ' + username);
                        console.log('<<< POSSIVEL ERRO  >>>' + username + ':' + e.message);
                    });
                }

                //Closing the current post modal
                await page.click(this.config.selectors.post_close_button)
                    .catch((e) => console.log('<<< ERRO FECHANDO O POST >>> ' + e.message));
                //Wait for random amount of time
                await page.waitFor(1000 + Math.floor(Math.random() * 250));
            }
        }
    };
    ...
    async unFollowUsers() {
        ...
        console.log("qtd usuarios firebase : " + Object.keys(following).length )
        ...
        if (following) {
            ...
            // users_to_unfollow = all_users;
        }

        console.log("vou parar de seguir " + users_to_unfollow.length + " usuarios")
        if (users_to_unfollow.length) {
            for (let n = 0; n < users_to_unfollow.length; n++) {
                ...
                try{
                    await this.page.waitForSelector(this.config.selectors.user_unfollow_button, {timeout: 5000})
                ...
                if (followStatus === 'Following') {
                    ...
                    console.log("deixei de seguir atu Firebase")
                    await this.firebase_db.unFollow(user);
                } else {
                    //save user to our following history
                    console.log("ja nao sigo atu Firebase")
